    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<PagedServiceResult<IEnumerable<ProductsWithGalleryViewModel>>> GetAllProductsByCategoryAsync(int page, int limit, int category = 0, string? search = "")
        {
            try
            {
                ...
                var productDtos = products.Data.Select(p => MapToProductWithGallery(p)).ToList();
        ...
        public async Task<ServiceResult<ProductsWithGalleryViewModel>> GetProductDetailsWithGalleryByIdAsync(int productId)
        {
            try
            {
                var product = await _productRepository.GetProductWithGalleryByIdAsync(productId);
                if (product == null)
                {
                    return ServiceResult<ProductsWithGalleryViewModel>.Failure($"Product with ID {productId} not found.");
                }

                var productDto = MapToProductWithGallery(product);
                return ServiceResult<ProductsWithGalleryViewModel>.Success(productDto);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error retrieving product with ID {productId}");
                return ServiceResult<ProductsWithGalleryViewModel>.Failure("An error occurred while retrieving the product");
            }
        }
        ...
        #region Mappers
        private ProductsWithGalleryViewModel MapToProductWithGallery(Product p)
        {
